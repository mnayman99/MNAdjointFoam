...
#include "objectiveDeltaLiftMN.H"
...
defineTypeNameAndDebug(objectiveDeltaLiftMN, 0);
addToRunTimeSelectionTable
(
    objectiveIncompressible,
    objectiveDeltaLiftMN,
    dictionary
);
...
objectiveDeltaLiftMN::objectiveDeltaLiftMN
(
    const fvMesh& mesh,
...
void objectiveDeltaLiftMN::update_meanValues()
{
...
void objectiveDeltaLiftMN::update_boundarydJdp()
{
    for (const label patchI : forcePatches_)
    {
        const fvPatch& patch = mesh_.boundary()[patchI];
        tmp<vectorField> tdx = patch.Cf() - rotationCentre_;
        bdJdpPtr_()[patchI] = (momentDirection_ ^ tdx)*invMomDenom_ - forceDirection_*invDenom_;
    }
}


void objectiveDeltaLiftMN::update_dSdbMultiplier()
{
...
void objectiveDeltaLiftMN::update_dxdbMultiplier()
{
    const volScalarField& p = vars_.p();
    const volVectorField& U = vars_.U();
...
void objectiveDeltaLiftMN::update_dxdbDirectMultiplier()
{
    const volScalarField& p = vars_.p();
...
void objectiveDeltaLiftMN::update_boundarydJdnut()
{
    const volVectorField& U = vars_.U();
...
void objectiveDeltaLiftMN::update_boundarydJdGradU()
{
    const autoPtr<incompressible::RASModelVariables>& turbVars =
        vars_.RASModelVariables();
    const singlePhaseTransportModel& lamTransp = vars_.laminarTransport();
